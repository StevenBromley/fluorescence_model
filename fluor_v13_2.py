...
#%%
def fluxes_with_profiles(raw_rad,raw_lines,raw_levs,lower_col,upper_col,aval_col,obj_vel,solar_dist,supp=True,bbtemp=5777):
    """
    ...
    for i in range(0,len(raw_lines[:,0])):     
        ...
        doppler_wave = doppler(wavelength_vac_nm,obj_vel)
        if ((supp == True) and ((doppler_wave < raw_rad[0,0]) or (doppler_wave > raw_rad[-1,0]))):
            ...
            bb = blackbody_wave(doppler_wave,temp=bbtemp) #Only need that specific wavelength at temp=5777K (default); assumes nm in
        ...
        if (raw_rad[0,0] < doppler_wave < raw_rad[-1,0]):
                ...
                searched_rad_indices = fniiv(raw_rad[:,0],doppler_wave,start_idx)
                #start_idx is passed to next iteration.
                start_idx = searched_rad_indices[1]
                #Index for the lines radiation field:
                rad_idx = searched_rad_indices[0]
                rad_at_wav = raw_rad[rad_idx,1] * 1e9 #converts from W/m^2 per nm to W /m^2 per meter (wavelength)
                integrated_fluxes[i] = rad_at_wav
    end = time.time()
