...
This dictionary contains utility codes related to FlorPy. They are largely intended for molecules, but some of the time-dependent codes are also contained here.
...
def separate_vib_bands(flor, element_string, orbit_id):
    ...
    """
    start = time.time()
    #Grab the transition array from florpy dict structure and generate an output dictionary:
...
def grab_trs_in_range(flor,element_string, xlow, xhigh):
    ...
    xlow and xhigh are the wavelength limits you want data between. 
    
    Units for wavelengths xlow and xhigh: nm
    
    Requires user has ran "separate_vib_bands" before running. 
...
def gen_band_lum(flor,element_string,orbit_id,up_state,low_state,vib_up,vib_low):
    ...
    Update June 2024: This function is now automated via "auto_gen_band_lum"
...
def parse_states(flor,element_string):
    """
    SJB
    
    This function is a post-processing code to pre-sort level indices into electronic and vibrational states. 
 
    User must pass in the flor dict after doing a fluorescence run and running separate_vib_bands().
    
    """
    lev_dat = flor[element_string]['levs_str']
    uniq_electronic_states = np.unique(lev_dat[:,0])
    #
    electronic_dict = {}
    for i in range(0,len(uniq_electronic_states[:])):
        print('Processing levels for electronic state "{:}"'.format(uniq_electronic_states[i]))
        #first, we want to save find the indices of the levels in each vibrational state.
        electronic_dict[uniq_electronic_states[i]]  = {}
        electronic_inds = np.where(lev_dat[:,0] == uniq_electronic_states[i])
        electronic_dict[uniq_electronic_states[i]]['electronic_indices'] = electronic_inds
        electronic_dict[uniq_electronic_states[i]]['vib_levels'] = {}
        electronic_dict[uniq_electronic_states[i]]['vib_levels']['indices'] = {}
        #
        uniq_vib_levs = np.unique(lev_dat[electronic_dict[uniq_electronic_states[i]]['electronic_indices'],1])
        vibs_out = np.zeros((len(uniq_vib_levs),2))
        vibs_out[:,0] = uniq_vib_levs
        for j in range(0,len(vibs_out[:,0])):
            #Trying to use clever methods has led to too much wasted time troubleshooting instead of just brute-forcing. Theres not a lot
            #of computation here, so Ill just brute-force it.
            num_levs = len(np.where((lev_dat[electronic_inds,1].astype(int)) == int(vibs_out[j,0]))[1]) 
            vib_inds = np.zeros((num_levs),dtype=int)
            vib_inds[:] = -1 #as a sanity check
            vib_count = 0
            for k in range(0,len(lev_dat[:,0])):
                if ( (lev_dat[k,0] == uniq_electronic_states[i]) and (int(lev_dat[k,1]) == int(vibs_out[j,0]) ) ):
                    vib_inds[vib_count] = k
                    vib_count = vib_count + 1
            electronic_dict[uniq_electronic_states[i]]['vib_levels']['indices'][j] = vib_inds
            lowest_e = lev_dat[vib_inds[0],3]
            print('v = {:}, E = {:}'.format(vibs_out[j,0],lowest_e))        
            vibs_out[j,1] = lowest_e
        electronic_dict[uniq_electronic_states[i]]['vib_levels']['vib_energies'] = vibs_out
    flor[element_string]['electronic_vib_inds'] = electronic_dict
    return(flor)
