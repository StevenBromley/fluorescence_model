...
#One can look at the 'gfacs' array, but it is not very human readable. We can reference it for plotting, but a more human-readable
# array can be generated with the provided function:
gfac_clean_array = generate_gfac_outfile(flor,element_string,orbit_id)    
#Save the g-factors to a file for other processing if desired:
np.savetxt('{:}_gfacs.txt'.format(gfac_file),gfac_clean_array,delimiter='\t', fmt = '%s')
...
plt.savefig('Sample CS stick spectrum.pdf',dpi=200)
...
#%%
#If desired, the user can compute the possible uncertainty in the gfactors by Monte-Carloing them. 
#Note: for CO+, all lines have an 'A' rating as a placeholder.
...
#   As an example, we will run 10 iterations.

number_of_err_iterations = 10
...
plt.legend()
plt.title('CS Fluorescence Spectrum with approximate errorbars based on {:} iterations'.format(number_of_err_iterations), fontsize=15)
plt.xlim(230,275)
plt.grid(True)
plt.ylabel(r'G-factor ($10^{-21}$ J/s/mol)',fontsize=15)
plt.xlabel('Wavelength (air nm)', fontsize=15)
plt.savefig('sample_CS_errorbar_plot.pdf', dpi=200)


#%%
# We grab the population array and use it to define the initial conditions for the time-dependent run:
