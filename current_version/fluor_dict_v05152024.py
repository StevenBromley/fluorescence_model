...
# Some useful constants. All required units are defined inside the functions where they are used
Rsun = 6.957e8 # meters
h = 6.62607004e-34
...
def generate_fluxes_general(flor,element_string,orbit_id,rad_choice='default', bbsupp=False, bbtemp = 5777, profiles='delta', lower_cutoff = 1e30, upper_cutoff=1e30, aval_min = 0, bbflux_min=0, permit_cascade=True):
    ...
    bbflux_min: set the minimum rate for inclusion in the model when using bbsupp=True. Not recommend for general use.
    ...
    but their pumping rates are set to 0. This allows transitions to be included if they act as weak perturbers or connect levels, but otherwise are not important in the rate equations.
            ...
            for key in flor[element_string]['rad_choices'][rad_choice]:
                ...
                for i in range(0,len(all_waves[:,0])):
                    if (flor[element_string]['rad_choices'][rad_choice][key]['lims'][0,0] < doppler(all_waves[i,wave_col],obj_vel) < flor[element_string]['rad_choices'][rad_choice][key]['lims'][0,1]):
                        ...
                        rad_at_wav = dist_scale_1au * flor[element_string]['rad_choices'][rad_choice][key]['fluxes'][rad_idx,1] * 1e9 #1e9 converts from W/m^2 per nm (wavelength) to W /m^2 per meter (wavelength)
            ...
            if (bbsupp == True):
                for i in range(0,len(integrated_fluxes[:])):
                    if (integrated_fluxes[i] == 0):
                        ...
                        bb = blackbody_wave(doppler(vac_waves[i],obj_vel),temp=bbtemp) #Only need that specific wavelength at temp=5777K (default); assumes nm input
...
#Following if check added on Nov29 in fix to blackbody test:
    if (rad_choice != 'blackbody'):
        for key in flor[element_string]['rad_choices'][rad_choice]:
...
def doppler(wavelength,velocity):
    ...
    shifted_wavelength = rest_wavelength * (1 - velocity/c)
...
def matrix_solve(lhs,rhs):
    ...
    if (zeros_check(pops) == False):
        ...
        #If the entire solution is negative, this is not a problem. However, if multiple negative multiplications are found, the solution is invalid.
        ...
        if (zeros_check(pops) == False): #This is executed if no zeros are in the solution. Proceed to check for negative populations
            ...
            for i in range(0,len(pops[:])):
                if (pops[i] < 0):
                    ...
                    neg_check = neg_check + 1
            if (neg_check > 1):                
                print('Problematic negative populations present in matrix solution. Attemping NNLS solution...')
            else:
                print('No negative populations persisting in matrix solution. Proceeding...')
                    
    if (neg_check > 1):
        print('Attempting SciPy Non-Negative Least Squares solver...')
        pops = nnls(lhs,rhs.reshape(-1),maxiter=1e5,atol=1e-3)[0] #Changed 1e10 -> 1e5
        neg_check = 0
        for i in range(0,len(pops[:])):
            if (pops[i] < 0):
                pops = pops * -1
...
def generate_index_scheme(lines,levs,fluxes,upper_col,lower_col,aval_col,ritz_col,lowerj_col,upperj_col):
    ...
    idx = np.zeros((len(lines[:,0]),7),dtype=float)
    ...
    for i in range(0,len(lines[:,0])):
        ...
        #freq = c / (wavelength_vac * 1e-9)
        g_i = 2 *float(levs[int(idx[i,0]),2]) + 1 #lower level degeneracy
        g_j = 2 *float(levs[int(idx[i,1]),2]) + 1 #upper level degeneracy
        aval = float(lines[i,aval_col]) #spont. emission rate
        stim_coeff = (wavelength_vac * 1e-9)**5 * aval * 1/(8 * np.pi * h * c**2)
        #For operating in frequency space with the default solar spectrum, one can use the Einstein B's from Schleicher (also uncomment 'freq' above):
#        stim_coeff = (1/(8*np.pi * h)) * (c**3) / (freq**3) * aval  #Schleicher; frequency space
        absorp_coeff = stim_coeff * g_j / g_i     #calc'd absorption coeff. from stim. coeff.
        rad_at_wav = fluxes[i]
        #   If one wants to use the Einstein B's in frequency space, an additional conversion factor must be added into the B's:
        #   To use frequency space, the other definition of stim_coeff above must be used, and the current rate must be multiplied by
        # the rad factor variable:   
#        rad_factor = (wavelength_vac * 1e-9)** 2 / (c**2)
        absorp_coeff = (wavelength_vac * 1e-9)**5 * aval * 1/(8 * np.pi * h * c**2) * g_j / g_i
        stim_coeff = absorp_coeff * g_i / g_j
        absorp_rate = absorp_coeff * rad_at_wav #* rad_factor
        stim_rate = stim_coeff * rad_at_wav #* rad_factor
        #Version for Schleicher frequency space units test:
        #absorp_rate = absorp_coeff * rad_at_wav * rad_factor
        #stim_rate = stim_coeff * rad_at_wav * rad_factor
...
def populate_lhs(lhs_empty,idx,rel_levs):
    ...
    return(lhs_empty)
#%%
def calc_line_intensities(pops,idx,rel_levs,geo_vel,renorm=False):
    """
    Updated 04-29-2022
    SJB
...
def calculate_pops_and_gfactors(flor,element_string,orbit_id,renorm=False):  
    ...
    pops = matrix_solve(lhs,rhs)   #Attempt to solve matrix equation using, in order, matrix inversion, pseudo-inverse, and NNLS
...
def generate_rad_grid(rad, wavelengths):
    ...
    return(output_arr)
#%%
def error_iterations(flor,element_string,orbit_id, bbsupp=False, bbtemp = 5777, lower_cutoff = 1e30, upper_cutoff=1e30, aval_min = 0, rad_choice = 'default', profiles = 'doppler', err_id = 'default_setting', num_iterations=100,process=True,permit_cascade=True):
    """
    08-10-2021
    SJB
...
def generate_constrained_lte_pops(flor,element_string,constrained_level_indices, vib_temp,rot_temp):
    """
    SJB
    May 2024
    This is a custom function for handling MOLECULES in florpy when some levels are described by a rotational and/or vibrational temperature.
    In this case, we have a series of energy levels that are fixed (relatively) w.r.t. other states. These constrained levels are described by
    the variables 'vib_temp' and 'rot_temp'

    Inputs: vib_temp - vibrational temperature in Kelvin
            rot_temp - rotational temperature in Kelvin
    
    Requires running separate_vib_bands and parse_states beforehand.
    
    As of June 2024, this is now self-contained in another function and does not need to be manually called. This just effectively computes LTE
    populations and returns them for use in the constrained solve method.
    """
    #Generate a final array:
    constrained_pops = np.zeros((len(constrained_level_indices[:]),2))
    for i in range(0,len(constrained_pops[:,0])):
        constrained_pops[i,0] = i
        
    vib_temp_cm = vib_temp * 0.695034800 #This is effectively kt where k is in units of cm inverse per Kelvin
    rot_temp_cm = rot_temp * 0.695034800 #This is effectively kt where k is in units of cm inverse per Kelvin
    lev_dat = flor[element_string]['levs_str']
    e_inds = flor[element_string]['electronic_vib_inds']
    #
    final_pops = np.zeros((len(lev_dat[:,0]),2))
    for i in range(0,len(final_pops[:,0])):
        final_pops[i,0] = i
    final_pops[:,1] = -1
    #We want to loop over the electronic states in the electronic_dict structure.
    #For each electronic state, we want to compute the boltzmann factor based on the vibrational energy.
    #Then, we want to loop over the rotational states and distribute that population according to the rotational temperature. We will implement this by
    #Computing the vibrational energy first, and then calculating the relative rotational populations. After that, we will normalize the rotational populations
    #to the value we calculated for the vibrational term. In the last step we'll renormalize the total population of all states to 1 just to ensure
    #that no numerical difficulties were encountered
    for key in e_inds:
        state_arr = flor[element_string]['electronic_vib_inds'][key]
        vib_pops = np.zeros((len(state_arr['vib_levels']['vib_energies'][:,0]),3))
        vib_pops[:,0] = state_arr['vib_levels']['vib_energies'][:,0]
        vib_pops[:,1] = state_arr['vib_levels']['vib_energies'][:,1]
        for key2 in state_arr['vib_levels']['indices']:
            vib_energy = state_arr['vib_levels']['vib_energies'][int(key2),1]
            vib_pops[int(key2),2] = np.exp(-1 * vib_energy/vib_temp_cm)        
        flor[element_string]['electronic_vib_inds'][key]['vib_pops'] = vib_pops
        #Now that the vibrational energies are worked out, we want to compute the rotational components.
        for key2 in state_arr['vib_levels']['indices']:
            rel_inds = state_arr['vib_levels']['indices'][key2]
            rot_pops = np.zeros((len(state_arr['vib_levels']['indices'][key2][:])))
            #Since the rotational temperature can sometimes be small, this can lead to overflow problems which return NaN for
            #the populations (not desirable at all). We'll get around it by considering properties of exponentials:
            # e^x * e^y = e^(y+x). By adding a constant offset in exponential, it will keep the numbers reasonably sized
            #Then when we renormalize afterward the extra factor cancels.
            #For the offset, we'll use the energy of the first rotational level:
            rot_offset = float(lev_dat[rel_inds[0],3]) / rot_temp_cm
            for i in range(0,len(rel_inds[:])):
                #In terms of J:
                rot_pops[i] = (2 * float(lev_dat[rel_inds[i],2]) + 1 ) * np.exp(-1 * (float(lev_dat[rel_inds[i],3])/rot_temp_cm - rot_offset))
                #In terms of N:
                #rot_pops[i] = (2 * narr[rel_inds[i]] + 1 ) * np.exp(-1 * (float(lev_dat[rel_inds[i],3])/rot_temp_cm - rot_offset))
            rot_sum = np.sum(rot_pops[:])
            rot_pops[:] = (rot_pops[:] / rot_sum) * vib_pops[int(key2),2]
            final_pops[rel_inds,1] = rot_pops
    #We loop over and grab only the relevant populations for our constrainted populations:
    for i in range(0,len(constrained_level_indices[:])):
        constrained_pops[i,1] = final_pops[int(constrained_level_indices[i]),1]
    popsum = np.sum(constrained_pops[:,1])
    constrained_pops[:,1] = constrained_pops[:,1] / popsum
    print('Sanity Check! Sum of Constrained Populations = {:}'.format(np.sum(constrained_pops[:,1])))
    return(constrained_pops)

def calculate_pops_and_gfactors_with_thermalized_states(flor,element_string,orbit_id,constrained_level_indices,vib_temp,rot_temp):  
    """
    SJB
    May 15 2024
    
    This function replaces 'calculate_pops_and_gfactors' in the case where some levels are fixed. This function assumes
    the levels indexed by variable 'constrained_level_indices' are described (in LTE) by the vibrational and rotational temperatures
    in variables vib_temp and rot_temp.
        
    The indices of the levels can be constructed manually, or grabbed from (for example):
        All of the levels in the electronic state 'X':        
        flor[element_string]['electronic_vib_inds']['X']['electronic_indices'][0]
    or:
        All of the levels in the X vibrational state v=0:
        flor[element_string]['electronic_vib_inds']['X']['vib_levels']['indices'][0]
        All of the levels in the X vibrational state v=1:
        flor[element_string]['electronic_vib_inds']['X']['vib_levels']['indices'][1]
        and so on...
        
    For most molecules, simply grabbing all of the levels of the ground electronic state is probably sufficient.
    The indices belonging to the constrained levels should be passed in the variable 'constrained level indices'
    
    Lastly, this function requires the user to have ran separate_vib_bands() and parse_states() beforehand.
    
    """
    constrained_pops = generate_constrained_lte_pops(flor,element_string,constrained_level_indices,vib_temp,rot_temp)
    #Define output dict structure:
    flor[element_string][orbit_id]['outputs'] = {}
    #Define arrays for lines/levels after we process them:
    lines = flor[element_string][orbit_id]['relevant_lines']
    ...
    geo_vel = flor[element_string][orbit_id]['orbit_params']['geo_vel_kms'] * 1000 #m/s required for doppler formula
    fluxes = flor[element_string][orbit_id]['fluxes']
    #We generate a new internal indexing scheme:
    model_indexing = generate_index_scheme(lines,levs,fluxes,upper_col,lower_col,aval_col,ritz_col, lowerj_col, upperj_col)
    idx = model_indexing[0] # The "idx" array contains all of the line information (indices, wavelengths, and 3 atomic rates) needed as input
    new_idx = model_indexing[1] #Number of levels involved in the model
    rel_levs = model_indexing[2] #Relevant levels for the model; this contains the original level indexing, an indicator if a level is in the final model, and the internal indexing scheme
    ...
    lhs = populate_lhs(lhs_empty,idx,rel_levs) #Pass in idx array (containing atomic data) and populate the LHS matrix (matrix A)
    rhs = np.zeros((len(lhs[:,0]),1)) #Generate the 'B' Matrix    
    #Now, we want to take the LHS matrix and trim down the LHS array by removing the rows and columns corresponding to the constrained levels.
    test2 = np.copy(lhs)
    new_lhs = np.delete(test2,constrained_level_indices,axis=0)
    new_lhs = np.delete(new_lhs,constrained_level_indices,axis=1)
    #We make a new array and work out the mapping from old_lhs -> new_lhs: 
    new_lev_count = 0
    rel_indexing = np.zeros((len(new_lhs[:,0]),2),dtype=int)
    for i in range(0,len(rel_levs[:,0])):
        if (i not in constrained_level_indices[:]):
            rel_indexing[new_lev_count,0] = i
            rel_indexing[new_lev_count,1] = new_lev_count
            new_lev_count = new_lev_count + 1
    #Now, we need to use the original lhs array and the constrained populations to generate the new rhs array.
    new_rhs = np.zeros((len(new_lhs[:,0]),1))
    #We loop over the new lhs array. For each element, we compute the contributions from the constrained levels and move them to the relevant location in the new_rhs array:
    for i in range(0,len(new_lhs[:,0])):
        new_index = rel_indexing[i,1]
        old_index = rel_indexing[i,0]
        rhs_val = 0
        for j in range(0,len(constrained_level_indices[:])):
            #We compute the sum of the old lhs[old index for level,j] * constrained population j:
            rhs_val = rhs_val + lhs[old_index,j] * constrained_pops[j,1]
        new_rhs[new_index] = -1 * rhs_val #-1 comes from moving from lHS to RHS
    unconstrained_pops = matrix_solve(new_lhs,new_rhs)   #Attempt to solve matrix equation using, in order, matrix inversion, pseudo-inverse, and NNLS
    #The following is not the most elegant, but is reasonably fast for our purposes. Reconstruct master population list and renormalize:
    final_pops = np.zeros((len(lhs)))
    con_pop = constrained_pops[:,1]
    final_pops[constrained_level_indices] = con_pop
    flattened_unconstrained_pops = unconstrained_pops.reshape(-1)
    final_pops[rel_indexing[:,0]] = flattened_unconstrained_pops
    renorm = np.sum(final_pops[:])
    final_pops = final_pops / renorm
    #Compute g-factors:
    line_intensities = calc_line_intensities(final_pops,idx,rel_levs,geo_vel,renorm=False)
    #Save outputs to the dictionary structure:
    flor[element_string][orbit_id]['outputs']['rates'] = idx
    flor[element_string][orbit_id]['outputs']['relevant_levels'] = rel_levs
    flor[element_string][orbit_id]['outputs']['pops'] = final_pops
    flor[element_string][orbit_id]['outputs']['gfactors'] = line_intensities   
    flor[element_string][orbit_id]['outputs']['lhs_matrix'] = lhs
    flor[element_string][orbit_id]['outputs']['rhs_matrix'] = rhs
    flor[element_string][orbit_id]['outputs']['new_lhs_matrix'] = new_lhs
    flor[element_string][orbit_id]['outputs']['new_rhs_matrix'] = new_rhs
    flor[element_string][orbit_id]['outputs']['trot'] = rot_temp
    flor[element_string][orbit_id]['outputs']['tvib'] = vib_temp
    flor[element_string][orbit_id]['outputs']['constrained_level_indices'] = constrained_level_indices
    flor[element_string][orbit_id]['outputs']['constrained_level_populations'] = constrained_pops
    flor[element_string][orbit_id]['outputs']['unconstrained_level_map'] = rel_indexing
    return(flor)


def convolve_spec(xrange,energies,ints,fwhm):
     """
     SJB 03-23-2021
     Simple function for convolving sticks with gaussians to produce synthetic spectra
     """
     convolved_spec = np.zeros((len(xrange),2))
     convolved_spec[:,0] = xrange
     for i in range(0,len(energies[:])):
         p0 = [ints[i],energies[i],fwhm]
         convolved_spec[:,1] = convolved_spec[:,1] + gaussian_fwhm(xrange,*p0)
     return(convolved_spec)

def gaussian_fwhm(x, *p):
     """
     SJB 2023
...
def generate_gfac_outfile(flor,element_string,orbit_id):
    """
    SJB 
    
    Updated June 2024
    
    This function processes the line list and gfactor information into a human readable array. 
    """
    
    trs_arr = flor[element_string][orbit_id]['relevant_lines_str']
    gfacs = flor[element_string][orbit_id]['outputs']['gfactors']
    #
    gfac_array = np.zeros((len(trs_arr[:,0])+1,len(trs_arr[0,:])+2),dtype=object)
    #Define Header information:
    gfac_array[0,0] = 'Wavelength (air nm)'
    gfac_array[0,1] = 'Wavelength (vac nm)'
    gfac_array[0,2] = 'Gfactor (J / s / mol)'
    gfac_array[0,3] = 'Einstein A Coeff. (1/s)'
    gfac_array[0,4] = 'Lower level Energy (1/cm)'
    gfac_array[0,5] = 'Upper level Energy (1/cm)'
    gfac_array[0,6] = 'Accuracy'
    gfac_array[0,7] = 'Lower Electronic State'
    gfac_array[0,8] = 'Lower Spin State'
    gfac_array[0,9] = 'Lower Vibrational State'
    gfac_array[0,10] = 'Lower Rotational Quantum Number (N)'
    gfac_array[0,11] = 'Lower Total Angular Momentum (J)'
    gfac_array[0,12] = 'Upper Electronic State'
    gfac_array[0,13] = 'Upper Spin State'
    gfac_array[0,14] = 'Upper Vibrational State'
    gfac_array[0,15] = 'Upper Rotational Quantum Number (N)'
    gfac_array[0,16] = 'Upper Total Angular Momentum (J)'
    gfac_array[0,17] = 'Band Label'
    #
    gfac_array[1:,0:3] = gfacs
    gfac_array[1:,3:] = trs_arr[:,1:]
    return(gfac_array)
