...
def fluorescence_spectra(fluxes,raw_lines,raw_levs,ritz_col,lower_col,upper_col,aval_col,renorm=True):  
    """
    Updated on 02-05-2021 to enforce particle conservation.
    ...
    for i in range(0,len(lines[:,0])):
        ...
        #updated on 01-29-2021; fixed units. Old:
        #stim_coeff = aval * np.pi**2 * c**3 / (hbar * omega**3)
        #New:
        #stim_coeff = aval * np.pi**2 * c**2 / (hbar * omega**3)
        #Added 01-29-2021 to switch to wavelength units:
    ...
    print('{:} Transitions Connecting {:} out of {:} possible levels'.format(len(raw_lines),new_idx,len(levs[:,0])))
    """
    ...
    pop_end = time.time()
    print('Rate Matrices Populated in {:} seconds'.format(round(pop_end - pop_start,4)))
    print('Starting Matrix Solution...')
    #New approach; utilize particle conservation for ground state, e.g. entire first row summed = 1
    rhs = np.zeros((len(lhs[:,0]),1))
    rhs[0] = 1
    #Set entire first row of lhs to 1:
    ...
    print('Intensity Calculations finished in {:} seconds'.format(round(intens_end - solution_end,4)))
    return(pops,intens,idx)
